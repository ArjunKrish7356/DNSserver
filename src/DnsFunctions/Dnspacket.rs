...
pub fn handle_query(socket: &UdpSocket) -> Result<(), E> {
    let mut buf = BytePacketBuffer::new();
    let (_, src) = socket.recv_from(&mut buf.buf)?;

    // create a new dns packet and set the values of it from the query_packet
    let mut query_packet = DnsPacket::from_buffer(&mut buf)?;
    let mut packet = DnsPacket::new();
    packet.header.id = query_packet.header.id;
    packet.header.recursion_desired = true;
    packet.header.questions = 1;
    packet.header.recursion_available = true;
    packet.header.response = true;

    if let Some(question) = query_packet.questions.pop() {
    ...
    let buf_len = res_buf.pos();
    let data = res_buf.get_range(0, buf_len)?;
